...
function wp_invoice_backend_wrap($title = false, $content = false) {
	?>
	<div class='wrap'>
...
*/
	function wp_invoice_invoice_row($invoice, $page) {
		...
		if($invoice->is_archived) $class_settings .= " wp_invoice_archived ";
...
*/	
	function wpi_checkbox($args = '', $checked = false) {
		...
		if(strpos("$name",'[') || strpos("$name",']')) {
			$replace_variables = array('][',']','[');
			$class_from_name = $name;
			$class_from_name = "wp_invoice_" . str_replace($replace_variables, '_', $class_from_name);
		} else {
			$class_from_name = "wp_invoice_" . $name;	
		}
