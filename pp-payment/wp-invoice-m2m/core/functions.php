...
/*
wp_invoice_create("post_id=2&payer_id=2&payee_id=1&amount=30.00&status=good&type=type");
wp_invoice_create("post_id=2&payer_id=2&payee_id=1&amount=40.00&status=good&type=type");
wp_invoice_create("post_id=2&payer_id=3&payee_id=1&amount=650.00&status=good&type=type");
wp_invoice_create("post_id=2&payer_id=3&payee_id=2&amount=22.00&status=good&type=type");
wp_invoice_create("post_id=2&payer_id=5&payee_id=3&amount=42.00&status=good&type=type");
...
function wp_invoice_user_settings($what, $user_id = false) {
	...
	$user_settings = get_usermeta($user_id, 'wp_invoice_settings');
...
function wp_invoice_create($args) {
	global $blog_id, $wpdb;
...
/*
	Prepares ajax data for cc transaction
*/
function wp_invoice_process_cc_ajax() {

	$nonce = $_REQUEST['wp_invoice_process_cc'];
	$invoice_id = $_REQUEST['invoice_id'];

 	if (! wp_verify_nonce($nonce, 'wp_invoice_process_cc_' . $invoice_id) ) die('Security check'); 

	wp_invoice_process_cc_transaction();
}

function wp_invoice_process_cc_transaction($cc_data = false) {
...
function wp_invoice_user_accepted_payments($payee_id) {

	if(wp_invoice_user_settings('paypal_allow', $payee_id) == 'true')
		$return[paypal_allow] = true;

	if(wp_invoice_user_settings('cc_allow', $payee_id) == 'true')
		$return[cc_allow] = true;

	if(wp_invoice_user_settings('draft_allow', $payee_id) == 'true')
...
function wp_invoice_email_variables($invoice_id) {
	...
	$wp_invoice_email_variables = array(
		'business_name' => $invoice_info->payee_class->nickname,
