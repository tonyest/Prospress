...
define("WP_INVOICE_VERSION_NUM", "0.7");
...
*/
	...
	function send_invoice() {
		...
		if($has_invoice_permissions) {
			...
			if($invoice->is_paid) {
				$paid_data = $wpdb->get_row("SELECT value, time_stamp FROM  ".WP_Invoice::tablename('log')." WHERE action_type = 'paid' AND invoice_id = '".$invoice_id."' ORDER BY time_stamp DESC LIMIT 0, 1");			
	...
	function incoming_invoices() {
		...
		$incoming_invoices = $wpdb->get_col("SELECT id FROM ".WP_Invoice::tablename('m2m')." WHERE payer_id = '$user_ID'");
	...
	function outgoing_invoices() {		
		...
		global $wpdb, $user_ID, $wp_invoice_page_names;
		$outgoing_invoices = $wpdb->get_col("SELECT id FROM ".WP_Invoice::tablename('m2m')." WHERE payee_id = '$user_ID'");
	...
	function init() {
		...
		if(!get_usermeta($user_ID, 'wp_invoice_settings')) {
			//wpi_qc("Running wp_invoice_load_default_user_settings() from init()");
			wp_invoice_load_default_user_settings($user_ID);
		}
		
		// Load these variables early
		$this->incoming_invoices = $wpdb->get_col("SELECT id FROM ".WP_Invoice::tablename('m2m')." WHERE payer_id = '$user_ID'");
		$this->outgoing_invoices = $wpdb->get_col("SELECT id FROM ".WP_Invoice::tablename('m2m')." WHERE payee_id = '$user_ID'");
		...
		if($wpdb->get_var("SHOW TABLES LIKE '". WP_Invoice::tablename('m2m') ."'") != WP_Invoice::tablename('m2m')) {
			$sql_main = "CREATE TABLE ". WP_Invoice::tablename('m2m') ." (
