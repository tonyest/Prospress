...
function pp_insert_mocks() {
	...
	for( $i = 1; $i < 6; $i++){
		...
		$wpdb->insert( $wpdb->posts, array( 'post_author' => $user_one, 'post_date' => date ("Y-m-d H:i:s"), 'post_date_gmt' => date ("Y-m-d H:i:s"), 'post_content' => $post_content, 'post_title' => $post_title ), array( '%d', '%s', '%s', '%s', '%s' ) );
		...
		for( $b = $i; $b < $i + 5; $b++ ){
			if( $b == $i + 4 ){ // last bid on a post, make it the winning bid
				...
				error_log("Inserted bid $bid_id on post $post_id");
			} else {
				$bid_stat = "outbid";
...
function call_generate_invoice( $post_id ) { //receive post ID from hook
	...
	error_log( 'schedule end post hook called call_generate_invoice with post id = ' . $post_id );
	$winning_bid = get_winning_bid( $post_id );
	$payer_id = $winning_bid->bidder_id;
	$payer_name = get_userdata( $payer_id )->display_name;
	$payee_id = get_post( $post_id )->post_author;
	...
	require_once( WP_PLUGIN_DIR . "/wp-invoice-m2m/core/functions.php" );

	$args = compact( 'post_id', 'payer_id', 'payee_id', 'amount', 'status', 'type' );
	error_log( 'wp_invoice_create being called with = ' . print_r( $args, true ) );
	//wp_invoice_create("post_id=2&payer_id=2&payee_id=1&amount=30.00&status=good&type=type");
	wp_invoice_create( $args );
}
...
function pp_payment_deactivate() {
	...
	foreach( $mock_post_ids as $post_to_delete ){
		...
		$wpdb->query( $sql );
		$sql = "DELETE FROM wp_invoice_m2m WHERE  post_id = " . $post_to_delete . ";";
		error_log('Running sql = ' . print_r($sql, true));
		$wpdb->query( $sql );
	}
	...
	//$sql = "DROP TABLE IF EXISTS " . $wpdb->bids;
	error_log('Running sql = ' . print_r($sql, true));
